package vn.nganluong.naba.channel.vib.service.impl;
...
import vn.nganluong.naba.channel.vib.controller.VIBIBFTController;
...
public class ReconciliationServiceImpl implements ReconciliationService {
	...
	private static final Logger logger = LogManager.getLogger(ReconciliationServiceImpl.class);
	...
	@Override
	public boolean doDayReconciliationVIBIBFT(String... dates) {
		...
		try {
			...
			PgFunction pgFunction = pgFunctionRepository
					.findFirstByCodeAndStatus(VIBConst.FUNCTION_CODE_IBFT_RECONCILIATION_DAY, true);

			PgSftp pgSftp = pgSftpRepository.findTopByChannelIdAndPgFunctionId(channelFunction.getChannel().getId(),
					pgFunction.getId());
			...
			if (filesDownloaded.size() > 0) {
				...
				doiSoatERPGiaoDichNgay(channelFunction, pgSftp, filesDownloaded, dateInFileName, dates);
				return  true;
	 ...
	 */
	private void doiSoatERPGiaoDichNgay(ChannelFunction channelFunction, PgSftp pgSftp, List<String> filesDownloaded,
		...
		if (dates == null || dates.length < 1) {
			...
			// fromDateString = MyDateUtil.toDateString(VIBConst.VIB_IBFT_RECONCILIATION_TRAN_TIME_CSV_FORMAT_DATE,
			//		fromDate);

			// 23h ngày hôm qua trừ 1 MILLISECOND
			toDate = MyDateUtil.getYesterDay23Hour(); // DateUtils.addHours(DateUtils.truncate(new Date(), Calendar.DAY_OF_MONTH), -1);
			// toDateString = MyDateUtil.toDateString(VIBConst.VIB_IBFT_RECONCILIATION_TRAN_TIME_CSV_FORMAT_DATE, toDate);
		} else {
...
//				Locale.ENGLISH).format(toDate);
		...
		List<ReconciliationDayDataCompareDTO> reconciliationResultList = createReconciliationResultList(
				channelFunction.getChannel().getId(), fromDate, toDate, fileName);
	...
	@Override
	public boolean doMonthReconciliationVIBIBFT(String month) {
		try {

			ChannelFunction channelFunction = channelFunctionService.findChannelFunctionByCodeAndChannelCode(
					VIBConst.CHANNEL_FUNCTION_STATUS_TRANSACTION, VIBConst.CHANNEL_CODE);
			PgFunction pgFunction = pgFunctionRepository
					.findFirstByCodeAndStatus(VIBConst.FUNCTION_CODE_IBFT_RECONCILIATION_MONTH, true);
			...
			if (filesDownloaded.size() > 0) {
				...
				String detailDiffFileName = doiSoatERPChiTietGiaoDichThang(channelFunction, pgSftp, startMonth, endMonth, monthInFileName);
				System.out.println(detailDiffFileName);
				
				// Thực hiện đối soát chi tiết tháng.
				String summaryDiffFileName = doiSoatERPTongHopGiaoDichThang(channelFunction, pgSftp, startMonth, endMonth, monthInFileName);
	 ...
	 * @throws IOException
	 */
	private String doiSoatERPChiTietGiaoDichThang(ChannelFunction channelFunction, PgSftp pgSftp, Date fromDate, Date toDate, String dateFileName)
			throws IOException {
	 ...
	 * @throws IOException
	 */
	private String doiSoatERPTongHopGiaoDichThang(ChannelFunction channelFunction, PgSftp pgSftp, Date fromDate, Date toDate, String dateFileName)
			throws IOException {
