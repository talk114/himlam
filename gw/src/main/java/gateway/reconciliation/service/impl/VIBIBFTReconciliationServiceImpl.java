...
//
//@Service
//public class VIBIBFTReconciliationServiceImpl extends PaymentGate implements VIBIBFTReconciliationService {
//
//    private static final Logger logger = LogManager.getLogger(VIBIBFTReconciliationServiceImpl.class);
//
//    @Autowired
...
//
//    @Override
//    public ResponseEntity<PGResponse> doDayReconciliationVIBIBFT(PGRequest pgRequest) {
//        try {
//            DayReconciliationReq dayReconciliationReq = objectMapper.readValue(pgRequest.getData(), DayReconciliationReq.class);
//            String[] dates = null;
...
//            PgFunction pgFunction = pgFunctionRepository.findFirstByCodeAndStatus(VIBConst.FUNCTION_CODE_IBFT_RECONCILIATION_DAY, true);
//
...
//                logger.info(commonLogService.createContentLog(VIBConst.CHANNEL_CODE, VIBConst.SERVICE_NAME_IBFT, VIBConst.FUNCTION_CODE_IBFT_RECONCILIATION_DAY, false, false, true, paramsLog));
...
//     */
//    private void doiSoatERPGiaoDichNgay(Integer channelId, PgSftp pgSftp, List<String> filesDownloaded, String dateInFileName, String... dates) throws IOException, ParseException {
//        Date fromDate = null;
//        Date toDate = null;
//        if (dates == null || dates.length < 1) {
//            // 23h ngày hôm kia
//            fromDate = MyDateUtil.getBeforeYesterDay23Hour();
//            // 23h ngày hôm qua trừ 1 MILLISECOND
//            toDate = MyDateUtil.getYesterDay23Hour();
//        } else {
//            // From date: Trừ đi 1 tiếng (23 ngày hôm trước)
//            fromDate = DateUtils.addHours(DateUtils.parseDate(dates[0], VIBConst.VIB_IBFT_PARAMS_FORMAT_DATE), -1);
//            // Todate: Trừ đi 1 tiếng
//            Date toDateTemp = DateUtils.setHours(DateUtils.parseDate(dates[1], VIBConst.VIB_IBFT_PARAMS_FORMAT_DATE), 23);
//            // Todate trừ đi 1ms để so sánh:Thời gian: 23:00:00 (T-2) đến 22:59:59 (T-1).
...
//                List<List<String>> listData = (List<List<String>>) fileService.readFileTextToList(fileName, VIBConst.ERP_RECONCILIATION_SEPERATOR_CHAR_IN_FILE);
...
//
//    @Override
//    public ResponseEntity<PGResponse> doMonthReconciliationVIBIBFT(PGRequest pgRequest) {
//        try {
//            MonthReconciliationReq reconciliationReq = objectMapper.readValue(pgRequest.getData(), MonthReconciliationReq.class);
//
//            String month = reconciliationReq.getMonth();
//            Channel channel = channelRepository.findFirstByCodeAndStatus(pgRequest.getChannelName(), true);
//            PgFunction pgFunction = pgFunctionRepository.findFirstByCodeAndStatus(VIBConst.FUNCTION_CODE_IBFT_RECONCILIATION_MONTH, true);
//            PgSftp pgSftp = pgSftpRepository.findTopByChannelIdAndPgFunctionId(channel.getId(), pgFunction.getId());
//
...
//
//    /**
//     * Đối soát file giao dịch tháng Trả về tên file csv đã tạo sau khi đối
//     * soát.
//     *
//     * @param channelId
//     * @param pgSftp
...
//     * @throws IOException
//     */
//    private String doiSoatERPChiTietGiaoDichThang(Integer channelId, PgSftp pgSftp, Date fromDate, Date toDate, String dateFileName) throws IOException {
//        String fileName = pgSftp.getLocalDirIn() + StringUtils.split(pgSftp.getLocalSyntaxFileNameIn(), '|')[1].replace("{0}", dateFileName);
//        List<ReconciliationDayDataCompareDTO> differencePayment = createReconciliationResultList(channelId, fromDate, toDate, fileName);
//        String fileNameReconResultOutput = pgSftp.getLocalDirOut() + StringUtils.split(pgSftp.getLocalSyntaxFileNameOut(), '|')[1].replace("{0}", dateFileName);
...
//     * @throws IOException
//     */
//    private String doiSoatERPTongHopGiaoDichThang(Integer channelId, PgSftp pgSftp, Date fromDate, Date toDate, String dateFileName) throws IOException {
//        String fileName = pgSftp.getLocalDirIn() + StringUtils.split(pgSftp.getLocalSyntaxFileNameIn(), '|')[0].replace("{0}", dateFileName);
//        List<ReconciliationMonthDataCompareDTO> differencePayment = createReconciliationMonthResultList(channelId, fromDate, toDate, fileName);
//        String fileNameReconMonthSummaryOutput = pgSftp.getLocalDirOut() + StringUtils.split(pgSftp.getLocalSyntaxFileNameOut(), '|')[0].replace("{0}", dateFileName);
...
//            }
//            // Check time
//            Date tranTime = MyDateUtil.parseDateFormat(VIBConst.ERP_RECONCILIATION_FILE_STRUCT_FORMAT_TIME + VIBConst.ERP_RECONCILIATION_FILE_STRUCT_FORMAT_DATE, row.get(8) + row.get(9));
...
//            dataRecord.setToAcct(row.get(4)); // TK nhận
...
//        }
//        List<ReconciliationDayDataCompareDTO> paymentsPg = paymentRepository.findByChannelAndDateAndPaymentType(
//                channelId, EnumPaymentType.ACCOUNT_NO.code(),
...
//                .noneMatch(o2 -> o2.getClientTranId().equals(o1.getClientTranId())
//                && o2.getTranAmt().compareTo(o1.getTranAmt()) == 0
//                && StringUtils.equals(o2.getToAcct(), o1.getToAcct())))
...
//
//            String[] timeReport = MyDateUtil.toDateString(VIBConst.ERP_RECONCILIATION_FILE_STRUCT_FORMAT_DATE + " " + VIBConst.ERP_RECONCILIATION_FILE_STRUCT_FORMAT_TIME, new Date()).split(" ");
//
//            footerRecon.setTranHourString(timeReport[1]);
//            footerRecon.setTranDateString(timeReport[0]);
//
//            footerRecon.setChecksum(HashUtils.hashingMD5(footerRecon.getRecordType() + footerRecon.getTotalRecord() + footerRecon.getCreator() + footerRecon.getTranHourString() + footerRecon.getTranDateString()));
//            reconResultList.add(footerRecon);
//        }
//        return reconResultList;
//    }
//
//    private List<ReconciliationMonthDataCompareDTO> createReconciliationMonthResultList(Integer channelId, Date fromDate, Date toDate, String fileName) {
...
//            }
//            // Checksum:
//            String[] fieldsToHas = new String[]{row.get(0), row.get(1), row.get(2), row.get(3), row.get(4), row.get(5), row.get(6), row.get(7)};
//            String dataToHas = StringUtils.join(fieldsToHas);
...
//                continue;
//            }
//            Date fromTranTime = MyDateUtil.parseDateFormat(VIBConst.ERP_RECONCILIATION_MONTH_FILE_STRUCT_FORMAT_DATE, row.get(3));
//            Date toTranTime = MyDateUtil.parseDateFormat(VIBConst.ERP_RECONCILIATION_MONTH_FILE_STRUCT_FORMAT_DATE, row.get(4));
//
...
//                .getSummaryPaymentByChannelAndDateAndPaymentType(channelId, EnumPaymentType.ACCOUNT_NO.code(),
//                        PaymentConst.EnumBankStatus.SUCCEEDED.code(), PaymentConst.EnumBankStatus.SUCCEEDED.code(),
//                        fromDate, toDate);
//
...
//                ReconciliationMonthDataCompareDTO pgUpdate = paymentsPg.stream().filter(p -> p.getTranType().equals(record.getTranType())).findFirst().orElse(null);
//                if (pgUpdate != null) {
...
//     * @throws IOException
//     */
//    private void doiSoatSoPhu(ChannelFunction channelFunction, PgSftp pgSftp, Date dateRc, String dateInFileName) throws IOException {
...
//
//        @SuppressWarnings("unchecked")
//        List<List<String>> listData = (List<List<String>>) excelService.readFileExcelToList(pgSftp.getLocalDirIn() + fileName);
//        for (List<String> row : listData) {
//            Date tranTime = MyDateUtil.parseDateFormat(VIBConst.VIB_IBFT_RECONCILIATION_TRAN_TIME_FORMAT_DATE, row.get(0));
...
//        List<ReconciliationDayDataCompareDTO> paymentsPg = paymentRepository.findByChannelAndDateAndPaymentType(
//                channelFunction.getChannel().getId(), EnumPaymentType.ACCOUNT_NO.code(),
//                PaymentConst.EnumBankStatus.SUCCEEDED.code(), PaymentConst.EnumBankStatus.SUCCEEDED.code(),
...
//    }
//
//    public boolean createReconDetailOutputFile(String fileName, List<ReconciliationDayDataCompareDTO> reconResultList) throws IOException {
...
//                StringUtils.trimToEmpty(row.getFromAcct()), StringUtils.trimToEmpty(row.getToAcct()), StringUtils.trimToEmpty(row.getTranAmtString()),
...
//                    .append(seperatorChar)
//                    .append(StringUtils.trimToEmpty(row.getToAcct()))
//                    .append(seperatorChar)
...
//    }
//
//    public boolean createReconMonthOuputFile(String fileName, List<ReconciliationMonthDataCompareDTO> reconResultList) throws IOException {
//        String seperatorChar = "|";
//        List<String> stringList = new ArrayList<String>();
//
...
//                printer.printRecord(
//                        MyDateUtil.toDateString(VIBConst.VIB_IBFT_RECONCILIATION_TRAN_TIME_CSV_FORMAT_DATE, item.getTranTime()),
